...
// $id = $_GET['id'];
	...
	$total_ars =mssql_fetch_assoc(mssql_query("SELECT count(*) as total_ars 
												...
												(select kd_box from msbox where kd_rak in 
													(select kd_rak FROM msrak where kd_lokasi = '".$id."'))"));


	$total_box = mssql_fetch_assoc(mssql_query("SELECT count(*) as total_box FROM msbox 
												WHERE kd_rak in (select kd_rak FROM msrak where kd_lokasi = '".$id."')"));

	$total_rak = mssql_fetch_assoc(mssql_query("SELECT count(*) as total_rak FROM msrak where kd_lokasi = '".$id."'"));
...
<h3>Informasi Lokasi <?php echo "<b><i>'".$id."'</i></b>"; ?></h3>
<div><i class="ketmenu">QRCode Scan Result</i></div>

<table width="100%" class="table table">
<tbody>
	<tr><td width="30%"><i>Total Rak</i></td><td width="5%">:</td><td align="right"><?php echo $total_rak['total_rak']; ?></td></tr>
...
<thead>
	<tr>
		<th width="5%">No</th>
		<th width="8%">Kode Rak</th>
		<th width="12%">Jumlah Box</th>
	...
	<?php
		...
		$Query	=	mssql_query("SELECT row_number() over (order by id_rak) as no, kd_rak from msrak where kd_lokasi = '".$id."'");

		
		while($res	=	mssql_fetch_assoc($Query)){

		$jumbox = mssql_fetch_assoc(mssql_query("SELECT count(*) as jml_box FROM msbox WHERE kd_rak = '".$res['kd_rak']."'"));

		$jumars  = mssql_fetch_assoc(mssql_query("SELECT count(*) as jml_ars FROM arsip 
													WHERE kd_box in 
													(select kd_box from msbox where kd_rak = '".$res['kd_rak']."')"));

		?>
		<tr>
			<td><?php echo $res['no'];?></td>
			<td><?php echo $res['kd_rak'];?></td>
			<td><?php echo $jumbox['jml_box']; ?></td>
